...
void QNNFlatten::ReshapeTranspose(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    int inputTempIndex = -1;
    int outputTempIndex = -1;
    int tempNum = 0;
    ...
    if (permuteInput) {
        ...
        mInputs.clear();
        mOutputs.clear();
        this->createParamTensor("perm", QNN_DATATYPE_UINT_32, {(uint32_t)inputPerm.size()}, (void *)inputPerm.data(), "_input_transpose");
    ...
    if (permuteOutput) {
        ...
        mInputs.clear();
        mOutputs.clear();
        this->createParamTensor("perm", QNN_DATATYPE_UINT_32, {(uint32_t)outputPerm.size()}, (void *)outputPerm.data(), "_output_transpose");
        mParams.push_back(*(mParamTensorWrappers.back()->getNativeParam()));

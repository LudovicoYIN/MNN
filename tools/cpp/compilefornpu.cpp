...
void _getConstData(const Net* net, std::vector<MNN::Express::VARP> inputs, const std::set<int>& inputIndexes, const std::set<int>& outputIndexes, std::map<int, std::tuple<int, int, std::vector<int>, std::vector<char>>> &constTensorData, std::string srcpath) {
    ...
    for (int i = 0; i < allTensors.size(); i++) {
        ...
        if (inputsMap.find(name) != inputsMap.end()) {
            ...
            allTensors[i]->buffer().host = (uint8_t*)MNNMemoryAllocAlign(info->size * sizeof(float), MNN_MEMORY_ALIGN_DEFAULT);
...
static void _findAllConstTensorIndex(const Net* net, const std::set<int>& inputIndexes, const std::set<int>& outputIndexes, std::shared_ptr<Schedule::ScheduleInfo> sharedConst, std::vector<int> &constOpId, std::map<int, std::tuple<int, int, std::vector<int>, std::vector<char>>> *constTensorData){
    auto selectOps = _collectNeededOps(net, inputIndexes, outputIndexes);
...
static NetT *_replaceConstOp(const void* buffer, size_t bufferSize, std::map<int, std::tuple<int, int, std::vector<int>, std::vector<char>>> &constTensorData, std::vector<int> &constOpId) {
...
static int _compileWholeModule(std::vector<std::string> inputNames, std::vector<std::string> outputNames, std::vector<std::vector<MNN::Express::VARP>> inputs, const std::set<int>& inputIndexes, const std::set<int>& outputIndexes, const void* buffer, size_t bufferSize, std::string srcpath, std::string dstMNNPath){
    int npuIndex = 0;
    ...
    int index = 0;
    for (auto p : inputIndexes) {
        // CoreML Backend will name tensor as t + index
        attr->list->s[index] = std::string("t") + std::to_string(p);
        index++;
    }
    extra->attr.emplace_back(std::move(attr));
...
int main(int argc, const char* argv[]) {
    ...
    if(_needSplitNet(net, inputIndexes, outputIndexes) == false){
        return _compileWholeModule(inputNames, outputNames, inputs, inputIndexes, outputIndexes, bufferPair.first, bufferPair.second, srcMNN, dstMNN);
    }
    ...
    for (int inputIndex=0; inputIndex < inputs.size(); ++inputIndex) {
        auto bufferPairTmp = std::make_pair(BuilderTmp[inputIndex].GetBufferPointer(), BuilderTmp[inputIndex].GetSize());
        ...
        for (int i=0; i<subModulesInfo.size(); ++i) {
            if (!subModulesInfo[i].isBreak) {
                ...
                npuOps[i] = std::move(_compileSubModule(moduleIO[i], subModulesInfo[i], bufferPairTmp.first, bufferPairTmp.second, srcPath, srcMNN, path, cpuTotal, npuTotal, inputIndex, graphicName));
                npuIndex++;
            }
        }

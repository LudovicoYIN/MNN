...
class LlmConfig(PretrainedConfig):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        # Qwen3.5/Qwen3 mixed stacks can mark non-full layers as
        # `linear_attention`; reuse the existing mixed-attention path.
        sliding_attn_layers = []
        if hasattr(llm_config, 'layer_types') and llm_config.layer_types:
            for i, layer_type in enumerate(llm_config.layer_types):
                if layer_type in ('sliding_attention', 'linear_attention'):
